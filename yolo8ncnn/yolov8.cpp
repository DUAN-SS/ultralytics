...
int Detect::DetectYolov8(const cv::Mat& bgr, std::vector<Object>& objects)
{
	ncnn::Net yolov8;
	...
	// stride 8
	{
		...
		GenerateProposals(8, out, prob_threshold, objects8);
	...
	// stride 16
	{
		...
		GenerateProposals(16, out, prob_threshold, objects16);
	...
	// stride 32
	{
		...
		GenerateProposals(32, out, prob_threshold, objects32);

		proposals.insert(proposals.end(), objects32.begin(), objects32.end());
	}

	NonMaxSuppression(proposals, objects,
		img_h, img_w, hpad / 2, wpad / 2,
		scale, scale, prob_threshold, nms_threshold);
	return 0;
}

float Detect::SoftMax(const float* src, float* dst, int length)
{
	float alpha = -FLT_MAX;
...
void Detect::GenerateProposals(int stride, const ncnn::Mat& feat_blob, const float prob_threshold, std::vector<Object>& objects)
{
...
float Detect::Clamp(float val,float min,float max)
{
	return val > min ? (val < max ? val : max) : min;
}
void Detect::NonMaxSuppression(
	std::vector<Object>& proposals,
